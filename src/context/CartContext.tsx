...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const addToCart = (item: CartItem) => {
    setCartItems((prev) => {
      const existingItem = prev.find(
        (i) => i.collection_sr_no_id === item.collection_sr_no_id
      );
      if (existingItem) {
        return prev.map((i) =>
  ...
  const removeFromCart = (collection_sr_no_id: number) => {
    setCartItems((prev) =>
      prev.filter((i) => i.collection_sr_no_id !== collection_sr_no_id)
    );
  };
  ...
  const updateQuantity = (collection_sr_no_id: number, quantity: number) => {
    ...
    setCartItems((prev) =>
      prev.map((i) =>
        i.collection_sr_no_id === collection_sr_no_id ? { ...i, quantity } : i
      )
    );
  };
